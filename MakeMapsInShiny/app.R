...
source("MakeMapsFromData_Final.R")
source("CLIWOCLeaflet_Final.R")
source("SVLeaflet_Final.R")
...
server <- function(input, output, session) {
  ...
  output$mymap1 <- renderLeaflet(
    if (all(textFns1() == "CLIWOC") & !is.null(textFns1())) {
      #print(textFns1())
      #print("making CLIWOC")
      makeCLIWOCmap("CLIWOCforLeaflet.csv", "CLIWOCSeaCoords.csv")
    }
    else if (all(textFns1() == "SVPost1750RawNames.csv", na.rm = TRUE) & !is.null(textFns1())) {
  ...
  output$mymap2 <- renderLeaflet(
    if (all(textFns2() == "CLIWOC") & !is.null(textFns2())) {
      makeCLIWOCmap("CLIWOCforLeaflet.csv", "CLIWOCSeaCoords.csv")
    }
    else if (all(textFns2() == "SVPost1750RawNames.csv") & !is.null(textFns2())) {
